...
```{r}
pheno<-data.table::fread("gen_data/02_activities/data/gwa.A2.fam")
print(pheno)
#4000 samples
```
...
```{r}
gen<-data.table::fread("gen_data/02_activities/data/gwa.A2.bim")
print(gen)
#306102 SNPs
```
...
```{r}
p3<-ggplot(pheno, aes(x = V6)) +
geom_density(fill="green", color="black")+
labs(x="Phenotype", y="Density")
#Yes, it looks roughly Gaussian

ggsave("gen_data/02_activities/data/p3_plot.png", plot = p3, width = 7, height = 5, dpi = 300)
```
...
```{bash}
plink2 --bfile ./gen_data/02_activities/data/gwa.A2 --maf 0.05 --geno 0.01 --mind 0.1 --hwe 0.00005 --make-bed --out ./gen_data/02_activities/data/gwa.qc.A2
```
...
```{bash}
plink2 --bfile ./gen_data/02_activities/data/gwa.qc.A2 --indep-pairwise 500 50 0.05 --out ./gen_data/02_activities/data/gwa.qc_pruned.A2

plink2 --bfile ./gen_data/02_activities/data/gwa.qc.A2 --extract ./gen_data/02_activities/data/gwa.qc_pruned.A2.prune.in --make-bed --out ./gen_data/02_activities/data/gwa.qc_pruned.A2
```
...
```{bash}
plink2 --bfile ./gen_data/02_activities/data/gwa.qc_pruned.A2 --king-cutoff 0.0884 --out ./gen_data/02_activities/data/gwa_king.A2
```
...
```{bash}
plink2 --bfile ./gen_data/02_activities/data/gwa.qc.A2 --keep ./gen_data/02_activities/data/gwa_king.A2.king.cutoff.in.id --make-bed --out ./gen_data/02_activities/data/gwa.qc_unrelated.A2
```
...
```{bash}
plink2 --bfile ./gen_data/02_activities/data/gwa.qc_unrelated.A2 --extract ./gen_data/02_activities/data/gwa.qc_pruned.A2.prune.in --make-bed --out ./gen_data/02_activities/data/gwa.qc_unrelated_pruned.A2

plink2 --bfile ./gen_data/02_activities/data/gwa.qc_unrelated_pruned.A2 --pca 20 --out ./gen_data/02_activities/data/gwa_unrel_PCA.A2
```
...
```{bash}
plink2 --bfile ./gen_data/02_activities/data/gwa.qc.A2 --ci 0.95 --linear --covar ./gen_data/02_activities/data/gwa_unrel_PCA.A2.eigenvec --covar-name PC1,PC2,PC3 --adjust --out ./gen_data/02_activities/data/gwa.qc_assoc.A2
```

(ii) Create a Manhattan plot of the GWAS results.

```{r}
assoc <- data.table::fread("gen_data/02_activities/data/gwa.qc_assoc.A2.PHENO1.glm.linear")
...
ggsave("gen_data/02_activities/data/p4_plot.png", plot = p4, width = 7, height = 5, dpi = 300)
```
