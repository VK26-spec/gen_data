...
```{bash}         
wc -l < ./gen_data/02_activities/data/gwa.qc.A1.fam
...
``` {bash}    
head ./gen_data/02_activities/data/gwa.qc.A1.fam
...
```{bash}         
wc -l < ./gen_data/02_activities/data/gwa.qc.A1.bim
...
```{bash} 
printf "%s\n" rs1861 rs3813199 rs3128342 rs11804831 > ./gen_data/02_activities/data/snplist.txt

plink2 --bfile ./gen_data/02_activities/data/gwa.qc.A1 --extract ./gen_data/02_activities/data/snplist.txt --freq --out ./gen_data/02_activities/data/gwa_qc_A1_freq
...
head(freq)
#rs3813199 AF is 0.0569126
#rs11804831 AF is 0.154341
#rs3128342 AF is 0.304121
#rs1861 AF is 0.0539859
```

(ii) What are the minor allele frequencies (MAFs) for these four SNPs?

```{bash}       
plink2 --bfile ./gen_data/02_activities/data/gwa.qc.A1 --extract ./gen_data/02_activities/data/snplist.txt --freq --maf 0.5 --out ./gen_data/02_activities/data/gwa_qc_A1_maf
```
```{r,message=FALSE, warning=FALSE}
...
```{bash}         
plink2 --bfile ./gen_data/02_activities/data/gwa.qc.A1 --hardy --out ./gen_data/02_activities/data/gwa_qc_A1_hwe
```
```{r,message=FALSE, warning=FALSE}
# View HWE results
hwe <- fread("gen_data/02_activities/data/gwa_qc_A1_hwe.hardy")
head(hwe)
```
...
```{bash}
plink2 --bfile ./gen_data/02_activities/data/gwa.qc.A1 --extract ./gen_data/02_activities/data/snplist.txt --hardy --out ./gen_data/02_activities/data/gwa_qc_A1_subset_hwe
head ./gen_data/02_activities/data/gwa_qc_A1_subset_hwe.hardy
...
```{bash}       
plink2 --bfile ./gen_data/02_activities/data/gwa.qc.A1 --extract ./gen_data/02_activities/data/snplist.txt --recode A --out ./gen_data/02_activities/data/geno.additive
```
```{r,message=FALSE, warning=FALSE}
# Load genotype data
geno_subset <- fread("gen_data/02_activities/data/geno.additive.raw")
...
```         
The regression coefficient for rs1861 is 0.96698 (p <2e-16). This indicates that each additional copy of the C allele is almost 1 to 1 linearly associated with increased odds of being a case. The significant p-value suggests evidence of an association between this SNP and the phenotype in our sample.
...
# Round phenotype values to the nearest integer
...
print(p1)
ggsave("gen_data/02_activities/data/p1_plot.png", plot = p1, width = 7, height = 5, dpi = 300)
```
...
``` {r,message=FALSE, warning=FALSE}           
model <- glm(PHENOTYPE ~ rs1861_C, data = geno_subset_dom, family = "gaussian")
summary(model)
#The p-value < 2.2e-16
```
...
print(p2)
ggsave("./gen_data/02_activities/data/p2_plot.png", plot = p2, width = 7, height = 5, dpi = 300)
```

(vii) Which model fits better? Justify your answer.

```         
The AIC is 11000 under the recessive model and 10988 under the additive model. This shows that while, the AICs are similar the additive model has a slightly better overall fit.
```
